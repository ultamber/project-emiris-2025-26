                     ...
                     const GroundTruth *groundTruth)
{
    ...
    for (int qi = 0; qi < Q; ++qi)
    {
        ...
        keepApprox = (int)distApprox.size();
                       ...
                       std::vector<std::vector<float>> &centroids, unsigned seed)
{
    ...
    for (int t = 1; t < k; ++t)
    {
        ...
        // Binary search for r
        int r = std::lower_bound(partialSums.begin(), partialSums.end(), x) - partialSums.begin();
        r = std::min(r, N - 1);
